...
func TestHandleHead_Integration(t *testing.T) {
	...
	headHandler := NewHeadHandler(usecase.NewGetter(retriever))
	...
	testCases := []struct {
		name           string
		setup          func(t *testing.T, minioClient *minio.Client, db *database.Database, minioEndpoint string, serverURL string) (*http.Request, int64, string)
		paramHash      string
		expectedStatus int
		checkResponse  func(t *testing.T, resp *http.Response, expectedSize int64, expectedType string)
	}{
		{
			name: "Successful retrieval of head",
			setup: func(t *testing.T, minioClient *minio.Client, db *database.Database, minioEndpoint string, serverURL string) (*http.Request, int64, string) {
				t.Helper()
				...
				_, err := minioClient.PutObject(context.Background(), minioBucket, hexHash,
				...
				blob := &model.Blob{
					ID:     hexHash,
					Bucket: minioBucket,
					MinIOAddress: fmt.Sprintf("http://%s/%s/%s", minioEndpoint,
						minioBucket, hexHash),
					UploadTime: time.Now(),
				...
				req := httptest.NewRequest(http.MethodHead, "/"+hexHash, http.NoBody)
				req.Header.Set(presentation.AuthKey, generateValidAuthHeader(t, 600, "head", hexHash, serverURL))

				return req, int64(len(content)), blobType
			},
			paramHash:      "",
		...
		{
			name: "Blob not found for head",
			setup: func(t *testing.T, _ *minio.Client, _ *database.Database, _ string, serverURL string) (*http.Request, int64, string) {
				t.Helper()
				nonExistentHash := "nonexistenthashforhead"
				req := httptest.NewRequest(http.MethodHead, "/"+nonExistentHash, http.NoBody)
				req.Header.Set(presentation.AuthKey, generateValidAuthHeader(t, 600, "head", nonExistentHash, serverURL))

				return req, 0, ""
			},
			paramHash:      "nonexistenthashforhead",
			expectedStatus: http.StatusNotFound,
			checkResponse: func(t *testing.T, resp *http.Response, _ int64,
				_ string,
			) {
				t.Helper()
		...
		{
			name: "Hash with extension for head",
			setup: func(t *testing.T, minioClient *minio.Client, db *database.Database, minioEndpoint string, serverURL string) (*http.Request, int64, string) {
				t.Helper()
				content := []byte("content with extension for head")
				hashBytes := sha256.Sum256(content)
				hexHash := hex.EncodeToString(hashBytes[:])
				blobType := "audio/mp3"
				...
				req := httptest.NewRequest(http.MethodHead, fmt.Sprintf("/%s.mp3", hexHash), http.NoBody)
				req.Header.Set(presentation.AuthKey, generateValidAuthHeader(t, 600, "head", hexHash, serverURL))
			...
			paramHash:      ".mp3",
			expectedStatus: http.StatusOK,
			checkResponse: func(t *testing.T, resp *http.Response, expectedSize int64,
				expectedType string,
			) {
				t.Helper()
				assert.Equal(t, fmt.Sprintf("%d", expectedSize),
		...
		},
		{
			...
			setup: func(t *testing.T, _ *minio.Client, _ *database.Database, _ string, serverURL string) (*http.Request, int64, string) {
				...
				req := httptest.NewRequest(http.MethodHead, "/"+hash, http.NoBody)
				req.Header.Set(presentation.AuthKey, generateValidAuthHeader(t, -600, "head", hash, serverURL))
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			req, size, bType := tc.setup(t, minioClient, db, minioEndpoint, serverURL)
			rec := httptest.NewRecorder()
			e.ServeHTTP(rec, req)

			assert.Equal(t, tc.expectedStatus, rec.Code)
			tc.checkResponse(t, rec.Result(), size, bType)
		})
	}
}
